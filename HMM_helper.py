...
def sample_sonnet_line(hmm, obs_map, syl_dic, syl=10):
    # Get reverse map.
    obs_map_r = obs_map_reverser(obs_map)

    # Sample and convert sentence.
    emission, states = hmm.generate_sonnet_emission(syl_dic, syl)
    sentence = [obs_map_r[i] for i in emission]

    return ' '.join(sentence).capitalize()

def sample_rhyme_line(hmm, obs_map, syl_dic, rhyme_dic, syl=10, rhyme = None):
    # Get reverse map.
    obs_map_r = obs_map_reverser(obs_map)

    # Sample and convert sentence.
    emission, states = hmm.generate_rhyme_emission(obs_map, syl_dic, rhyme_dic, syl, rhyme)
    sentence = [obs_map_r[i] for i in emission]
