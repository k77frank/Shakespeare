...
class HiddenMarkovModel:
    ...
    def __init__(self, A, O, sonnet=False):
        '''
    ...
    def generate_sonnet_emission(self, syl_dic, syl):
        ...
        while count < syl:
            ...
            while True:
                ...
                while (rand_var > 0):
                    rand_var -= self.O[state][next_obs]
                    next_obs += 1
                    if (count + int(syl_dic[next_obs-1]) <= 10) and (rand_var < (tip*0.00000001)):
                        break

                next_obs -= 1
                if (count + int(syl_dic[next_obs]) <= 10):
                    count += int(syl_dic[next_obs])
                    emission.append(next_obs)
                    break
                tip += 1
            # Sample next state.
    ...
    def generate_rhyme_emission(self, obs_map, syl_dic, rhyme_dic, syl, rhyme):
        import numpy as np
        import re
        ...
        if rhyme is None: 
            ...
            count = 0 
            last_syl = 0 
            last = np.random.choice(last_words)
            last = re.sub(r'[^\w]', '', last).lower()
            em = obs_map[last]
            emission.append(em)
            count += int(syl_dic[em])
            ...
            while count != syl:
                ...
                while True:
                    ...
                    while (rand_var > 0):
                        rand_var -= self.O[state][next_obs]
                        next_obs += 1
                        if (count + int(syl_dic[next_obs-1]) <= 10) and (rand_var < (tip*0.000001)):
                            break

                    next_obs -= 1
                    last_syl = int(syl_dic[next_obs])
                    if (count + int(syl_dic[next_obs]) <= 10):
                        count += int(syl_dic[next_obs])
                        emission.append(next_obs)
                        break
                    tip += 1
                # Sample next state.
                prev = list(np.array(self.A).transpose()[state])
                prev_sum = sum(prev)
                for i in range(len(prev)):
                    prev[i] /= prev_sum
                state = np.random.choice(range(self.L), p = prev)
            emission.reverse()

        else:
            last_words = rhyme_dic[rhyme]
